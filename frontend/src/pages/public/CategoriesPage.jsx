// src/pages/public/CategoriesPage.jsx — Mobile First
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      <div className="mb-7 sm:mb-10">
        <h1 className="font-display text-3xl sm:text-4xl font-bold text-gray-900 mb-1 sm:mb-2">หมวดหมู่อาหาร</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {cats.map(c => (
            <div key={c.id} onClick={() => navigate(`/foods?category=${c.id}`)}
              className="bg-white rounded-2xl shadow-sm p-5 sm:p-6 cursor-pointer hover:-translate-y-1 hover:shadow-lg transition-all active:scale-95 group">
              ...
              <div className="mt-3 sm:mt-4 text-xs sm:text-sm font-semibold transition-colors flex items-center gap-1"
                   style={{ color: c.color }}>
                ดูรายการ <span className="group-hover:translate-x-1 transition-transform inline-block">→</span>
              </div>
            </div>
          ))}
        </div>
